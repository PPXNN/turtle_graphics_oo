    ...
    def run(self):
        ...
        for i in range(30):
            ...
            if choice == 1:
                ...
                embedded = EmbeddedPolygon(3, size, orientation, location, color, border_size,num_levels,reduction_ratio)
                embedded.draw()
            elif choice == 6:
                embedded = EmbeddedPolygon(4, size, orientation, location, color, border_size,num_levels,reduction_ratio)
                embedded.draw()
            elif choice == 7:
                embedded = EmbeddedPolygon(5, size, orientation, location, color, border_size,num_levels,reduction_ratio)
                embedded.draw()
            elif choice == 8:
                num_sides = random.randint(3, 5)
                ...
                new_location = [random.randint(-300, 300), random.randint(-200, 200)]
                polygon = Polygon(num_sides, size, orientation, new_location, new_color, border_size)
                polygon.draw()
